...
class ObjectProjector(nn.Module):
    def __init__(self, feature_dim=512, object_dim=OBJECT_DIM):
        super().__init__()
...
class AttributeProjector(nn.Module):
    def __init__(self, feature_dim=512, attr_dim=ATTR_DIM):
        super().__init__()
        self.net = nn.Sequential(
...
class MITStatesBackbone(nn.Module):
    def __init__(self, feature_dim=512, object_dim=OBJECT_DIM, attr_dim=ATTR_DIM,
        ...
        self.attr_embed = nn.Embedding(num_attrs, attr_dim)
    ...
    def encode(self, x):
        ...
        return obj_emb, attr_emb

    def decode(self, z, injections=None):
        return self.decoder(z, injections)

    def forward(self, x, attr_idx, obj_idx):
        feature = self.encoder(x)
        obj_emb = self.object_proj(feature)
        attr_emb = self.attr_proj(feature)
        obj_anchor = self.object_embed(obj_idx)
        attr_anchor = self.attr_embed(attr_idx)
...
class DirectionalProbe(nn.Module):
    def __init__(self, object_dim=OBJECT_DIM, attr_dim=ATTR_DIM,
                 k_outputs=K_OUTPUTS, hidden_dim=256):
        super().__init__()
        self.k = k_outputs
...
class MITStatesCBDP(nn.Module):
    def __init__(self, feature_dim=512, object_dim=OBJECT_DIM, attr_dim=ATTR_DIM,
                 k_outputs=K_OUTPUTS, num_objects=50, num_attrs=50):
        super().__init__()
        self.backbone = MITStatesBackbone(feature_dim, object_dim, attr_dim,
                                          num_objects, num_attrs)
    ...
    def forward_convergent(self, x, attr_idx, obj_idx):
        return self.backbone(x, attr_idx, obj_idx)
